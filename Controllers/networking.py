...
from Controllers.quickrequest import quickRequest
...
class TopHatThread(Thread):
		...
		def run(self):
				while not self.stop:
						...
						# [0] = sock [1] = header [2] = self [3] = data [4] = uri [5] = opcode))
						...
						response = quickRequest(client, data[5], data[4], [3])
...
class ClientHandle(dispatcher):
		...
		def handle_read(self):
				...
				self.queue.put((self.sock,header,self,data,uri,opcode))
				return
